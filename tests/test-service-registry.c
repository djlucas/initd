...
void test_lookup_by_name(void) {
    ...
    register_service(6001, "webapp.service", KILL_PROCESS);
    register_service(6002, "database.service", KILL_CONTROL_GROUP);
...
void test_has_registry_capacity(void) {
    ...
    for (int i = 0; i < MAX_SERVICES - 1; i++) {
        char name[64];
        snprintf(name, sizeof(name), "service%d.service", i);
        register_service(7000 + i, name, KILL_PROCESS);
    }
    ...
    /* Add the last one */
    register_service(7999, "last.service", KILL_PROCESS);
    assert(service_registry_count() == MAX_SERVICES);
