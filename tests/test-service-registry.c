...
void test_register_service(void) {
    ...
    /* Register a service */
    int result = register_service(1234, "test.service", NULL, KILL_CONTROL_GROUP, -1, -1, 0);
    assert(result == 0);
...
void test_lookup_service(void) {
    ...
    /* Register a service */
    register_service(5678, "lookup-test.service", NULL, KILL_PROCESS, -1, -1, 0);
...
void test_lookup_nonexistent(void) {
    ...
    /* Register a service */
    register_service(1111, "exists.service", NULL, KILL_PROCESS, -1, -1, 0);
...
void test_unregister_service(void) {
    ...
    register_service(1000, "service1.service", NULL, KILL_PROCESS, -1, -1, 0);
    register_service(2000, "service2.service", NULL, KILL_CONTROL_GROUP, -1, -1, 0);
...
void test_multiple_services(void) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        int result = register_service(1000 + i, name, NULL, KILL_PROCESS, -1, -1, 0);
        assert(result == 0);
    }
...
void test_registry_full(void) {
    ...
    for (i = 0; i < MAX_SERVICES; i++) {
        char name[64];
        snprintf(name, sizeof(name), "service%d.service", i);
        int result = register_service(10000 + i, name, NULL, KILL_PROCESS, -1, -1, 0);
        assert(result == 0);
    }
    ...
    int result = register_service(99999, "overflow.service", NULL, KILL_PROCESS, -1, -1, 0);
    assert(result == -1);
    ...
    result = register_service(99999, "now-fits.service", NULL, KILL_PROCESS, -1, -1, 0);
    assert(result == 0);
    assert(service_registry_count() == MAX_SERVICES);
...
void test_reregister_same_pid(void) {
    ...
    /* Register a service */
    register_service(3000, "original.service", NULL, KILL_PROCESS, -1, -1, 0);

    /* Try to register with the same PID - will create a second entry
     * (this is intentional - PIDs can be reused by OS) */
    register_service(3000, "duplicate.service", NULL, KILL_CONTROL_GROUP, -1, -1, 0);
...
void test_kill_modes(void) {
    ...
    register_service(4001, "none.service", NULL, KILL_NONE, -1, -1, 0);
    register_service(4002, "process.service", NULL, KILL_PROCESS, -1, -1, 0);
    register_service(4003, "group.service", NULL, KILL_CONTROL_GROUP, -1, -1, 0);
    register_service(4004, "mixed.service", NULL, KILL_MIXED, -1, -1, 0);
...
void test_long_unit_name(void) {
    ...
    register_service(5000, long_name, NULL, KILL_PROCESS, -1, -1, 0);
...
void test_lookup_by_name(void) {
    ...
    register_service(6001, "webapp.service", NULL, KILL_PROCESS, -1, -1, 0);
    register_service(6002, "database.service", NULL, KILL_CONTROL_GROUP, -1, -1, 0);
...
void test_has_registry_capacity(void) {
    ...
    for (int i = 0; i < MAX_SERVICES - 1; i++) {
        char name[64];
        snprintf(name, sizeof(name), "service%d.service", i);
        register_service(7000 + i, name, NULL, KILL_PROCESS, -1, -1, 0);
    }
    ...
    /* Add the last one */
    register_service(7999, "last.service", NULL, KILL_PROCESS, -1, -1, 0);
    assert(service_registry_count() == MAX_SERVICES);
